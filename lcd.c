...
int init_LCD(void) {
	lcd = osThreadNew(THLCD, NULL, NULL); //Crear el Thread del timer
	if (lcd == NULL) {
    return(-1);
  }
	LCD_Init();
...
void THLCD(void *argument) {
	...
	while (1) {
			...
			//Iniciso 1: desplazar el LCD de la posición 9 a 109 por tiempo indefinido
//		do{
//			desplazarDerecha();
//			osDelay(100);
//			LCD_update();
...
//		}while(posicion > 9);
			...
			//Inciso 2: desplazar el LCD por medio del joystick con flags
//		flags = osThreadFlagsWait(0b11111, osFlagsWaitAny, osWaitForever);
//	//Up
//		switch(flags){
//			case 1:
//				limpiarBuffer();
...
//		LCD_update();
			...
			//Inciso 3: desplazar el LCD por medio del joystick con queues
//		status = osMessageQueueGet(queue_joystick, &msg, NULL, osWaitForever);
//		//Up
//		if(msg == 1){
//			limpiarBuffer();
//			posicionL1 = 109;
