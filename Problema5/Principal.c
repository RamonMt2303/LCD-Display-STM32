...
void lcd_Func(void *argument) {
	...
	while (1) {			
		status2 = osMessageQueueGet(queue_lcd, &valor, NULL, osWaitForever);   // Esperar Queue
		numero = valor + 48;
		status = osThreadFlagsWait((1 << 0) | (1 << 1) | (1 << 2), osFlagsWaitAny, osWaitForever); //Esperar Banderas
		LCD_clear();
		switch(status){
			case 1:
...
void rebotes_Func (void *argument) {
	...
	uint32_t status; //Flags de los Threads
  while (1) {
		status=osThreadFlagsWait(0x01,osFlagsWaitAny,osWaitForever); //Esperar Interrupcion
		switch (status){
			case 1:
...
void lcd_test_Func(void *argument) {
	...
	while (1) {			
		...
		numero = contador;
		osMessageQueuePut(queue_lcd, &numero, NULL, osWaitForever);
		osThreadFlagsSet(lcd, contador);
		osDelay(1000);
		contador++;
	}
}